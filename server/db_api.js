...
async function getOrderInfo(userCode) {
  var result = await sqlQuery(
    "select order_table.orderNumber,order_table.totalAmount,order_table.paymentTime,product_big_table.productName_c as productName_main,product_small_table.productName_c as productName_small,order_product_table.smallIndex,order_product_table.mainProductNumber,order_product_table.smallProductNumber from user_table,order_table,product_big_table,product_small_table,order_product_table where user_table.userCode=? and user_table.userCode=order_table.userCode and order_table.orderNumber = order_product_table.orderNumber and (order_table.orderStatus !='readyPayment')and order_product_table.mainProductCode=product_big_table.productCode and order_product_table.smallProductCode = product_small_table.productCode order by order_table.paymentTime DESC,product_big_table.productName_c,order_product_table.smallIndex,product_small_table.productName_c",
    [userCode]
  );
  return dbToJson(result);
}
